...
static void ob_sm3_process(ob_sm3_context *ctx, unsigned char data[64] )
{
...
void ob_sm3_finish(ob_sm3_context *ctx, unsigned char output[32] )
{
  ...
  ob_sm3_update( ctx, (unsigned char *) sm3_padding, padn );
  ob_sm3_update( ctx, msglen, 8 );
 ...
 */
void ob_sm3( unsigned char *input, int ilen,
          unsigned char output[32] )
{
  ob_sm3_context ctx;

  ob_sm3_starts( &ctx );
  ob_sm3_update( &ctx, input, ilen );
  ob_sm3_finish( &ctx, output );

	memset( &ctx, 0, sizeof(ob_sm3_context ) );
}
...
void ob_sm3_hmac_finish(ob_sm3_context *ctx, unsigned char output[32] )
{
  ...
  ob_sm3_finish( ctx, tmpbuf );
  ob_sm3_starts( ctx );
  ob_sm3_update( ctx, ctx->opad, 64 );
  ob_sm3_update( ctx, tmpbuf, hlen );
  ob_sm3_finish( ctx, output );
 ...
 */
void ob_sm3_hmac( unsigned char *key, int keylen,
               unsigned char *input, int ilen,
               unsigned char output[32] )
{
  ob_sm3_context ctx;

  ob_sm3_hmac_starts( &ctx, key, keylen);
  ob_sm3_hmac_update( &ctx, input, ilen );
  ob_sm3_hmac_finish( &ctx, output );
