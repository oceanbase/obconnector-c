...
static int ma_sm3_scramble(unsigned char *scramble, size_t scramble_len,
                              unsigned char *source, size_t source_len,
                              unsigned char *salt, size_t salt_len)
{
  unsigned char digest1[MA_SM3_HASH_SIZE],
                digest2[MA_SM3_HASH_SIZE],
                new_scramble[MA_SM3_HASH_SIZE];
  ...
  if (!(ctx= ma_hash_new(MA_HASH_SM3, ctx)))
    return 1;
  ...
  if (!(ctx= ma_hash_new(MA_HASH_SM3, ctx)))
    return 1;
  ma_hash_input(ctx, digest1, MA_HASH_SM3);
  ...
  if (!(ctx= ma_hash_new(MA_HASH_SM3, ctx)))
    return 1;
  ma_hash_input(ctx, digest2, MA_SM3_HASH_SIZE);
...
/* function prototypes */
static int auth_ob_sm3_client(MYSQL_PLUGIN_VIO *vio, MYSQL *mysql);
static int auth_ob_sm3_deinit(void);
static int auth_ob_sm3_init(char *unused1, size_t unused2, int unused3, va_list);


#ifndef PLUGIN_DYNAMIC
struct st_mysql_client_plugin_AUTHENTICATION ob_sm3_password_client_plugin=
#else
...
#endif
  unsigned char buf[MA_SM3_HASH_SIZE];
  ...
  */
  ...
  if (!is_connection_secure(mysql))
  {
     ...
     mysql->methods->set_error(mysql, CR_AUTH_PLUGIN_ERR, "HY000", 
                               "RSA Encryption not supported - ob_sm3_password plugin was built with GnuTLS support");
     return CR_ERROR;
#else
...
#if defined(HAVE_OPENSSL)
    bio= BIO_new_mem_buf(filebuffer ? (unsigned char *)filebuffer : packet, packet_length);
...
/*
   ...
   0           success
 */
static int auth_ob_sm3_init(char *unused1 __attribute__((unused)),
